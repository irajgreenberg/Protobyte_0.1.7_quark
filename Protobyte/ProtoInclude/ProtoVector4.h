...
namespace ijg {
    ...
    public:
        ...
        enum RandFlag {
            ...
            RANDOMIZED // use arg as ijg::random max value
        };
    ...
    inline ProtoVector4<T>::ProtoVector4(T xMin, T xMax, T yMin, T yMax, T zMin, T zMax, T wMin, T wMax){
        x = ijg::random(xMin, xMax);
        y = ijg::random(yMin, yMax);
        z = ijg::random(zMin, zMax);
        w = ijg::random(wMin, wMax);
    }
