...
void ProtoCamera::project(){
	...
	// stay consistent with gl's use of degrees
	float fh = tan(fovAngle / 360 * PI) * nearClipPlane;
	// temp stuff: REALLY code this stuff >GL 3.2 with Shaders/GLM
	float fw = fh * (viewPort.w / viewPort.h);
	glFrustum(-fw, fw, -fh, fh, nearClipPlane, farClipPlane);
	// reset MV matrix so lighting isn't screwed up
