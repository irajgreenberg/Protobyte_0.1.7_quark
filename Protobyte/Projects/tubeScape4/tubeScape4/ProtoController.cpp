...
void ProtoController::init() {
	...
	for (int i = 0; i < blockCount; i++) {
		...
		blockModel.setDiffuseMaterial({ .25, .25, .25, 1 });
		blockModel.setAmbientMaterial(0.0f);
		...
		blockModel.setTextureScale({ 1.0f, 1.0f });
		blockModel.setSpecularMaterial(1);
		blockModel.setShininess(70);

		blockModel.setRegistration(Block::BOTTOM);
	...
	partModel = Particle(
		...
		Particle::RECT,
		"iraWrapped_low.jpg");
		partModel.setSpeed(Vec(random(-2, 2), 0, random(-1, 1)));

	// particles
	for (int i = 0; i < particleCount; i++) {
		partPos.push_back(Vec{ random(-400, 400), 900, random(-1000, 0) });
		partSz.push_back(Dim3f(20, 20, 0));
