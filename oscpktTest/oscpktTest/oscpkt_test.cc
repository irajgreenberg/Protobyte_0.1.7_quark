...
void basicTests() {
  ...
  return;
  std::cout << "simplest message:\n"; hexdump(std::cout, wr.packetData(), wr.packetSize(), 0);std::cout << std::endl;
  assert(wr.isOk()); 
...
#define check(ok) do { if (!(ok)) {std::cerr << "TEST#" << test_number << " FAILED, line " << __LINE__ << ", " << #ok << "\n"; return false; } } while (0)
...
bool makeRandomTest(int seed, int fuzz=0, bool verbose=false) {
  ...
  if (verbose) std::cerr << "nb_item=" << nb_item << "\n";
  while (nb_item != 0 && bundle_recurs > 0) {
    if (prandom(7) == 0) { 
      if (verbose) std::cerr << "startBundle\n";
      ++bundle_recurs; wr.startBundle();
    } else if (prandom(11) == 0 && bundle_recurs) {
      if (verbose) std::cerr << "endBundle\n";
      ...
      if (verbose) std::cerr << "adding msg: " << msg << "\n";
      wr.addMessage(msg);
    }
    nb_item--;
  }
...
void randomTests(int nb_test, bool verbose) {
 std::cout << "doing random tests..." << std::endl;
  bool ok = true;
...
static void lo_osc_error(int num, const char *msg, const char *path) {
 std::cerr << "Error: liblo server error " << num
...
static int plop_handler(const char *path, const char *types, lo_arg **argv,
   ...
   std::cerr << "lo_server received a PLOP ! wouhou\n";
    return 0;
}
void libloTest() {
  UdpSocket sock;
 ...
 std::cout << "started lo_server on " << url << "\n";
  ...
  if (lo_server_recv_noblock(serv, 50) == 0) {
   std::cout << "reply not received by lo server\n";
  } else { 
   std::cout << "lo server got the reply!\n";
  }
}
#endif
...
void socketTests() {
  ...
  sock1.bindTo(0);
  if (!sock1.isOk()) std::cerr << "sock1: " << sock1.errorMessage() << "\n";

  sock2.connectTo(sock1.localHostName(), sock1.boundPort());
  if (!sock2.isOk()) std::cerr << "sock2: " << sock2.errorMessage() << "\n";

  for (int sz=8*8; sz <= 256*1024; sz *= 2) {    
    std::string buf(sz-32, 'x');
    bool sent_ok = sock2.sendPacket(buf.data(), buf.size());
    if (!sock2.isOk()) std::cerr << "sock2: " << sock2.errorMessage() << "\n";
	std::cerr << "sz=" << buf.size() << ", sent_ok=" << sent_ok << " ";
    if (sent_ok) {
      bool recv_ok = sock1.receiveNextPacket(100);
	  std::cerr << "recv_ok=" << recv_ok << ", sock1.receiveNextPacket: size=" << sock1.packetSize() << " from: " << sock1.packetOrigin();
      if (recv_ok) {
  ...
  std::cerr << "sock1 is replying to sock2\n";
  bool ok;
  ok = sock1.sendPacketTo("plop", 5, sock1.packetOrigin()); assert(ok);
  std::cerr << "sock2 is receiving the reply..\n";
  ok = sock2.receiveNextPacket(2000); assert(ok);
  std::cerr << "sock2 has received the reply, sz=" << sock2.packetSize() << ": '" << ((char*)sock2.packetData()) << "'\n";
...
void checkMatch(const char *pattern, const char *test, bool expected_match=true) {
	std::cout << "doing fullPatternMatch('" << pattern << "', '" << test << "'), expected result is : "
       << (expected_match?"MATCH":"MISMATCH") << std::endl;
  bool m = fullPatternMatch(pattern, test);
  if (!expected_match) {
    if (m) { std::cerr << "unexpected match... " << pattern << " with " << test << "\n"; assert(0); }
  } else {
    if (!m) { std::cerr << "unexpected mismatch... " << pattern << " with " << test << "\n"; assert(0); }
    std::string tmp(test);
...
void patternTests() {
	std::cout << "Doing pattern tests" << std::endl;
...
int main(int argc, char **argv) {
  ...
  std::cout << "OK it looks like everything works as expected!\n";
  return 0;
}
